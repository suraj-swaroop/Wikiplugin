#!/bin/bash
# - Name     : run_split_text.sh
# - Purpose  : split text files
# - Date     : 30, Mar, 2020
# - Author   : Donggu, Lee
#######################################################################
...
# File paths
SCRIPT_FILE_NAME='split_text_files.py'
...
if [[ "$#" -eq 2 || "$#" -eq 3 ]]; then
    ...
    echo "\$./run_split_texts.sh [YYYYMM] [Thread Count]"
    echo "\$./run_split_texts.sh 202001 5"
    echo "**************************************************************"
    exit 0   
fi
