#!/bin/bash
# - Name     : run_data_preprocess.sh
# - Purpose  : create an initial dataframe
# - Date     : 30, Mar, 2020
# - Author   : Donggu, Lee
#######################################################################
...
# File paths
SCRIPT_FILE_NAME='data_preprocess.py'
PATH_TO_SCRIPT=$PATH_TO_FILES$SCRIPT_FILE_NAME

WIKIPEDIA_TEXT_FILE=$HOME_PATH'/download/text'
...
function makeParameter()
{
    temp=""
    date=$1
    filePath=$WIKIPEDIA_TEXT_FILE"/"$date

    for path in "$filePath"/*.bz2
    do
        if [ "$temp" == "" ]; then
            temp=$path
        else
